...
Medical Microbiology/   ← V2 注册学科 2（医学微生物学，24 节点；从病原与感染性疾病 Ⅰ 拆分；2026-09-15 /ingest 第 1 章第一、二部分）
...
Pathology/              ← V2 注册学科 5（病理学，69 节点；2026-09-11 注册，2026-09-14 /ingest 第一、二章，2026-09-18 /ingest 第三章）
...
**双向链接的适用范围（2026-09-14 明确）**

| 关系 | 是否要求互链 | 说明 |
|------|--------------|------|
| 同级概念节点 ↔ 同级概念节点 | ✅ 应互链 | A 的 Related Concepts 列了 B，B 也应列 A |
| 导航页（MOC / README / Lecture）→ 概念节点 | ❌ 天然单向 | 枢纽页罗列所有子节点，**不要求**子节点回链枢纽 |
| 概念节点 → 所属章节 Lecture | ✅ 至少一次 | 便于从节点反查章节位置（章节导航即在该 Lecture 内） |

> **度量口径**：统计"未互惠出链"时**必须排除** MOC / README / Lecture 发出的链接，
> 否则枢纽页（如章节 Lecture 的 `## 本章知识导航`）会被误判为缺陷。

**链接写法（2026-09-14 实测 — 强制）**

链接默认用**裸基名** `[[Atrophy]]`（全库实测：裸基名 689 条 vs 相对路径 2 条）。
Lecture 正文统一 `详见 [[Node]]。`（一个或多个节点，紧跟小节标题之后；标题内不写链接）。
路径写法**仅限两个例外**：① Course / Lecture 的 `> - 关联学科：` 元数据；
② 指向 `02_Raw/` 来源文件（从 vault 根写起，不带 `../`）。
细则与反例见 [[99_System/Ingest-SOP.md|Ingest-SOP]] §3.4、后验项 §5.1.12。
